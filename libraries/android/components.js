...
prx.types.android_actionbar = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		if(typeof(item.text)!= "undefined" && item.text != "") {
			...
			cR += '<div class="caption liveUpdate-textColor changeProperty-textColor changeProperty-textSize changeProperty-textFont" style="line-height: '+_height+'px; '+getFontCssFromFontFamily(item.textFont)+' color: '+getColor(item.textColor)+'; font-size: '+item.textSize+'px; '+_props+'">'
...
prx.types.android_actionoverflow = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		cR += '<div class="actionOverflow">'
		cR += '<div class="AO AO1 liveUpdate-actionOverflowColor changeProperty-actionOverflowColor" style="width: '+_rectH+'px; height: '+_rectH+'px; background-color: '+getColor(item.actionOverflowColor)+'; border-radius: '+item.borderRadius+'px;"></div>'
		cR += '<div class="AO AO2 liveUpdate-actionOverflowColor changeProperty-actionOverflowColor" style="width: '+_rectH+'px; height: '+_rectH+'px; background-color: '+getColor(item.actionOverflowColor)+'; border-radius: '+item.borderRadius+'px;"></div>'
		cR += '<div class="AO AO3 liveUpdate-actionOverflowColor changeProperty-actionOverflowColor" style="width: '+_rectH+'px; height: '+_rectH+'px; background-color: '+getColor(item.actionOverflowColor)+'; border-radius: '+item.borderRadius+'px;"></div>'
		cR += '</div>'
...
prx.types.android_textarea = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		cR += '#'+_id+' textarea:focus { border-bottom-color: '+getColor(item.focusedBorderColor)+'!important; }'
		cR += '#'+_id+' textarea:focus ~ .left-edge, #'+_id+' textarea:focus ~ .right-edge { background: '+getColor(item.focusedBorderColor)+'!important; }'
		cR += '</style>'			
		...
		if(prx.editor) {
			...
			cR += '<textarea placeholder="'+item.placeholder+'" class="real-input changeProperty-backgroundColor changeProperty-textColor changeProperty-textSize changeProperty-textFont changeProperty-textAlign" style="color: '+getColor(item.textColor)+'; font-size: '+item.textSize+'px; '+getFontCssFromFontFamily(item.textFont)+' border-bottom: '+item.borderWidth+'px solid '+getColor(item.borderColor)+'; '+ _props + '; background-color: '+getColor(item.backgroundColor)+'; text-align: '+item.textAlign+';" data-role="none"></textarea>';
		}
		cR += '<span class="left-edge liveUpdate-borderColor-background-color" style="width: '+item.borderWidth+'px; background-color: ' + getColor(item.borderColor) + '"></span><span class="right-edge liveUpdate-borderColor-background-color" style="width: '+item.borderWidth+'px; background-color: ' +getColor(item.borderColor)+ '"></span>'
		cR += '</div>';
		return cR;
...
prx.types.android_onoffswitch = {
	...
	,onDisplay: function(item,containerid) {
		...
		cR += '<style>'
		cR += '#'+_id+' label .onoffswitch-inner { background-color: '+getColor(item.backgroundColor)+'; height: '+(item.height-4)+'px; '+getFontCssFromFontFamily(item.textFont)+'; font-size: '+Math.floor(item.textSize*percent)+'px; '+_props+'}'
		...
		cR += '#'+_id+' .inactivelabel .label-edge { border-color: '+getColor(item.inactiveLabelColor)+'; border-width: '+Math.ceil(item.height/2)+'px '+Math.ceil(item.width*0.1)+'px; right: -'+Math.floor(item.width*0.2)+'px; }';
...
prx.types.android_listcomplex = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		cR += '#' + _id + ' .android-list-inner { text-align: '+item.textAlign+'; '+cssFlexJustifyContent(item.textAlign)+' background-color: '+getColor(item.backgroundColor)+'; '+getFontCssFromFontFamily(item.textFont)+'; font-size: '+item.textSize+'px; color: '+getColor(item.textColor)+'; }';
		cR += '#' + _id + ' .bordered-container { border-top: '+item.dividerThickness+'px solid '+getColor(item.dividerColor)+'; }'
		cR += '#' + _id + ' .android-list-text { '+_props+' }';
		cR += '#' + _id + ' .android-list-subtitle { '+getFontCssFromFontFamily(item.subFont)+_subprops+' font-size: '+item.subSize+'px; color: '+getColor(item.subColor)+'; }'		
		cR += '#' + _id + ' input:checked + label { background: '+getColor(item.activeBackgroundColor)+'; color: '+getColor(item.activeTextColor)+'; }'		
		cR += '#' + _id + ' input:checked + label .android-list-subtitle { color: '+getColor(item.activeSubColor)+'; }'
		cR += '</style>';
		...
		$.each(item.listitems, function(i,elm){		
			...
			if(elm.itemtype == 'withCheckbox' || elm.itemtype == 'withRadio' || elm.itemtype == 'withIcon') {
				icon = '<div class="android-list-icon android-list-icon-'+item.iconpos+'" '+((typeof (elm.buttonicon) != "undefined" && elm.buttonicon.url != '') ? 'style="background-image: url('+ getAssetUrl(elm.buttonicon)+');"' : '' ) + '></div>';
			}
			...
			if(hasThumbs) {
				cR += '<div class="android-list-thumb" '+((typeof(elm.thumbnail) != "undefined" && elm.thumbnail.url != '') ? 'style="background-image: url('+ getAssetUrl(elm.thumbnail)+');"' : '') + '></div>';
			}
...
//TYPE = BASIC LIST
prx.types.android_listbasic = cloneobject(prx.types.android_listcomplex);
prx.types.android_listbasic.name = 'android_listbasic';
removeProperties(prx.types.android_listbasic.propertyGroups, ['iconpos', 'iconSize'])
removeProperties(prx.types.android_listbasic.dynamicProperties.propertyGroups, ['buttonicon','itemtype']);
prx.types.android_listbasic.dynamicProperties.blankItem = removeObjMembers(prx.types.android_listbasic.dynamicProperties.blankItem, ['buttonicon','checked']);
prx.types.android_listbasic.dynamicProperties.blankItem.itemtype = 'basic';
prx.types.android_listbasic.dynamicProperties.propertyGroups = editProperty(prx.types.android_listbasic.dynamicProperties.propertyGroups, 'itemtype', 'value', 'withSubtitle');
prx.types.android_listbasic.dynamicProperties.propertyGroups = editProperty(prx.types.android_listbasic.dynamicProperties.propertyGroups, 'itemtype', 'hiddenByDefault', true);

//TYPE = LIST WITH SUBTITLE
prx.types.android_listwithsubtitle = cloneobject(prx.types.android_listcomplex);
prx.types.android_listwithsubtitle.name = 'android_listwithsubtitle';
removeProperties(prx.types.android_listwithsubtitle.propertyGroups, ['iconpos', 'iconSize'])
removeProperties(prx.types.android_listwithsubtitle.dynamicProperties.propertyGroups, ['buttonicon','itemtype']);
prx.types.android_listwithsubtitle.dynamicProperties.blankItem = removeObjMembers(prx.types.android_listwithsubtitle.dynamicProperties.blankItem, ['buttonicon','checked']);
...
prx.types.android_listwithsubtitle.dynamicProperties.propertyGroups = editProperty(prx.types.android_listwithsubtitle.dynamicProperties.propertyGroups, 'itemtype', 'value', 'withSubtitle');
prx.types.android_listwithsubtitle.dynamicProperties.propertyGroups = editProperty(prx.types.android_listwithsubtitle.dynamicProperties.propertyGroups, 'itemtype', 'hiddenByDefault', true);

//TYPE = LIST WITH ICON
prx.types.android_listwithicon = cloneobject(prx.types.android_listcomplex);
prx.types.android_listwithicon.name = 'android_listwithicon';
removeProperties(prx.types.android_listwithicon.dynamicProperties.propertyGroups, ['itemtype']);
prx.types.android_listwithicon.dynamicProperties.blankItem = removeObjMembers(prx.types.android_listwithicon.dynamicProperties.blankItem, ['checked']);
...
prx.types.android_listwithicon.dynamicProperties.propertyGroups = editProperty(prx.types.android_listwithicon.dynamicProperties.propertyGroups, 'itemtype', 'value', 'withIcon');
prx.types.android_listwithicon.dynamicProperties.propertyGroups = editProperty(prx.types.android_listwithicon.dynamicProperties.propertyGroups, 'itemtype', 'hiddenByDefault', true);
prx.types.android_listwithicon.dynamicProperties.propertyGroups = editProperty(prx.types.android_listwithicon.dynamicProperties.propertyGroups, 'buttonicon', 'hiddenByDefault', false);

//TYPE = LIST WITH ICON AND SUBTITLE
prx.types.android_listwithiconandsubtitle = cloneobject(prx.types.android_listwithicon);
...
prx.types.android_listwiththumbnail = cloneobject(prx.types.android_listcomplex);
prx.types.android_listwiththumbnail.name = 'android_listwiththumbnail';
removeProperties(prx.types.android_listwiththumbnail.propertyGroups, ['iconpos', 'iconSize']);
removeProperties(prx.types.android_listwiththumbnail.dynamicProperties.propertyGroups, ['buttonicon','itemtype']);
prx.types.android_listwiththumbnail.dynamicProperties.blankItem = removeObjMembers(prx.types.android_listwiththumbnail.dynamicProperties.blankItem, ['buttonicon','checked']);
...
prx.types.android_listwiththumbnail.dynamicProperties.propertyGroups = editProperty(prx.types.android_listwiththumbnail.dynamicProperties.propertyGroups, 'itemtype', 'value', 'withThumnail');
prx.types.android_listwiththumbnail.dynamicProperties.propertyGroups = editProperty(prx.types.android_listwiththumbnail.dynamicProperties.propertyGroups, 'itemtype', 'hiddenByDefault', true);

//TYPE = LIST WITH THUMBNAIL AND SUBTITLE
prx.types.android_listwiththumbnailandsubtitle = cloneobject(prx.types.android_listwiththumbnail);
...
//TYPE = RADIO LIST
prx.types.android_listradio = cloneobject(prx.types.android_listcomplex);
...
prx.types.android_listradio.dynamicProperties.propertyGroups = editProperty(prx.types.android_listradio.dynamicProperties.propertyGroups, 'itemtype', 'value', 'withRadio');
prx.types.android_listradio.dynamicProperties.propertyGroups = editProperty(prx.types.android_listradio.dynamicProperties.propertyGroups, 'itemtype', 'hiddenByDefault', true);
prx.types.android_listradio.dynamicProperties.propertyGroups = editProperty(prx.types.android_listradio.dynamicProperties.propertyGroups, 'checked', 'hiddenByDefault', false);

//TYPE = CHECKBOX LIST
prx.types.android_listcheckbox = cloneobject(prx.types.android_listradio);
...
prx.types.android_listcheckbox.dynamicProperties.propertyGroups = editProperty(prx.types.android_listcheckbox.dynamicProperties.propertyGroups, 'itemtype', 'value', 'withCheckbox');
...
prx.types.android_notification_item = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		cR += '<style>';
		cR += '#' + _id + ' .android-notification-item-wrapper { background-color: '+getColor(item.backgroundColor)+'; }';
		if(item.thumbnail.url != '') {
			cR += '#' + _id + ' .android-notification-item-thumbnail { background-image: url('+getAssetUrl(item.thumbnail)+'); background-size: '+item.thumbnailSize*20+'% auto; background-color: '+getColor(item.thumbnailBgColor)+'; width: '+(dims.height)+'px; }'
		}
		cR += '#' + _id + ' .android-notification-item-title { color: '+getColor(item.titleColor)+'; '+getFontCssFromFontFamily(item.titleFont)+' font-size: '+item.titleSize+'px; '+_props+'}';
		cR += '#' + _id + ' .android-notification-item-subtitle { color: '+getColor(item.subtitleColor)+'; '+getFontCssFromFontFamily(item.subtitleFont)+' font-size: '+item.subtitleSize+'px; '+_subtitle_props+'}';
		cR += '#' + _id + ' .android-notification-item-time { color: '+getColor(item.timeColor)+'; '+getFontCssFromFontFamily(item.timeFont)+' font-size: '+item.timeSize+'px; font-weight: '+item.timeProperties+';}';
		if(item.imgSrc.url != '') {
		cR += '#' + _id + ' .android-notification-item-icon { background-image: url('+getAssetUrl(item.imgSrc)+'); background-size: '+item.iconSize*20+'% auto; }';
		}
		cR += '</style>';
...
prx.types.android_crouton = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		cR += '<style>';
		cR += '#' + _id + ' .android-crouton-wrapper { background-color: '+getColor(item.backgroundColor)+'; }';
		cR += '#' + _id + ' .android-crouton-text { color: '+getColor(item.textColor)+'; '+getFontCssFromFontFamily(item.textFont)+' font-size: '+item.textSize+'px; '+_props+'}';
		cR += '#' + _id + ' .android-crouton-close-separator { background-color: '+getColor(item.separatorColor)+';}';
		if(item.closeIcon.url != '') {
			cR += '#' + _id + ' .android-crouton-close-icon { width: '+Math.floor(dims.height*0.35)+'px; height: '+Math.floor(dims.height*0.35)+'px; background-image: url('+getAssetUrl(item.closeIcon)+');}';
		}
		cR += '</style>';
...
prx.types.android_card_small = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		cR += '<style>';
		cR += '#' + _id + ' .android-small-card-wrapper { background-color: '+getColor(item.backgroundColor)+'; box-shadow: 0 8px 6px -6px '+getColor(item.shadowColor)+'; }';
		cR += '#' + _id + ' .android-small-card-text { color: '+getColor(item.textColor)+'; '+getFontCssFromFontFamily(item.textFont)+' font-size: '+item.textSize+'px; '+_props+'}';
		if(item.thumbnail.url != '') {
			cR += '#' + _id + ' .android-small-card-thumbnail { background-image: url('+getAssetUrl(item.thumbnail)+'); background-size: '+item.thumbnailSize*20+'% auto; background-color: '+getColor(item.thumbnailBgColor)+'; width: '+dims.height+'px;}';
		}
		cR += '</style>';
...
prx.types.android_card_medium = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		cR += '<style>';
		cR += '#' + _id + ' .android-medium-card-wrapper { background-color: '+getColor(item.backgroundColor)+'; box-shadow: 0 8px 6px -6px '+getColor(item.shadowColor)+'; }';
		cR += '#' + _id + ' .android-medium-card-text { color: '+getColor(item.textColor)+'; '+getFontCssFromFontFamily(item.textFont)+' font-size: '+item.textSize+'px; '+_props+'}';
		if(item.thumbnail.url != '') {
			cR += '#' + _id + ' .android-medium-card-thumbnail { background-image: url('+getAssetUrl(item.thumbnail)+'); background-size: '+item.thumbnailSize*20+'% auto; background-color: '+getColor(item.thumbnailBgColor)+'; width: '+dims.width+'px; height: '+dims.width+'px;}';
		}
		cR += '</style>';
		...
		if(item.actionOverflow) {
			...
			cR += '<div class="AO AO1 liveUpdate-actionOverflowColor changeProperty-actionOverflowColor" style="width: 4px; height: 4px; background-color: '+getColor(item.actionOverflowColor)+';"></div>'
			cR += '<div class="AO AO2 liveUpdate-actionOverflowColor changeProperty-actionOverflowColor" style="width: 4px; height: 4px; background-color: '+getColor(item.actionOverflowColor)+';"></div>'
			cR += '<div class="AO AO3 liveUpdate-actionOverflowColor changeProperty-actionOverflowColor" style="width: 4px; height: 4px; background-color: '+getColor(item.actionOverflowColor)+';"></div>'
			cR += '</div>'
		}
		cR += '</div>';
		cR += '</div>';
	...
	,onResize: function(item,containerid) {
		var _id = (!containerid) ? item.id : containerid+'-'+item.id;
		var _width = getRealDims(item).width;
...
prx.types.android_card_big = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		cR += '<style>';
		cR += '#' + _id + ' .android-big-card-wrapper { background-color: '+getColor(item.backgroundColor)+'; box-shadow: 0 8px 6px -6px '+getColor(item.shadowColor)+'; }';
		cR += '#' + _id + ' .android-big-card-title { color: '+getColor(item.titleColor)+'; '+getFontCssFromFontFamily(item.titleFont)+' font-size: '+item.titleSize+'px; '+_props+'}';
		cR += '#' + _id + ' .android-big-card-subtitle { color: '+getColor(item.subtitleColor)+'; '+getFontCssFromFontFamily(item.subtitleFont)+' font-size: '+item.subtitleSize+'px; '+_subtitle_props+'}';
		if(item.thumbnail.url != '') {
			cR += '#' + _id + ' .android-big-card-thumbnail { background-image: url('+getAssetUrl(item.thumbnail)+'); background-size: '+item.thumbnailSize*20+'% auto; background-color: '+getColor(item.thumbnailBgColor)+'; width: '+dims.height+'px; height: '+dims.height+'px;}';
		}
		cR += '</style>';
...
prx.types.android_alert_dialog = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		cR += '<style>';
		cR += '#' + _id + ' .android-alert-dialog-wrapper { background-color: '+getColor(item.backgroundColor)+'; font-size: '+item.textSize+'px; }';
		cR += '#' + _id + ' .android-alert-dialog-text { color: '+getColor(item.textColor)+'; '+getFontCssFromFontFamily(item.textFont)+' font-size: '+item.textSize+'px; text-align: '+item.textAlign+'; '+_props+'}';
		cR += '#' + _id + ' .android-alert-dialog-buttons, #' + _id + ' .android-alert-dialog-button { border-color: '+getColor(item.borderColor)+'; border-width: '+item.borderWidth+'px; }';
		cR += '</style>';
		...
		$.each(item.buttons, function(i,elm) {
			...
			cR += '<div id="'+_id+'-buttons-'+i+'" class="android-alert-dialog-button dynamic-property liveUpdate-borderColor liveUpdate-textColor-'+i+' changeProperty-buttonColor" data-dynamic-property-index="'+i+'" style="'+_button_props+' color: '+getColor(elm.textColor)+';">';
			cR += '<span data-editableproperty="text" data-dynamic-property-index="'+i+'">' + elm.text + '</span>';
			cR += '</div>';
		});
		cR += '</div>';
...
prx.types.android_progressbasic = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		if(item.cancelIcon.url != '') {
			cR += '#' + _id + ' .android-progress-basic-icon { background-image: url('+getAssetUrl(item.cancelIcon)+'); background-size: '+(item.iconSize*20)+'% auto; }';
		}
		cR += '#' + _id + ' .android-progress-basic-wrapper { background-color: '+getColor(item.backgroundColor)+'; font-size: '+item.textSize+'px; }';
		cR += '#' + _id + ' .android-progress-basic-text { color: '+getColor(item.textColor)+'; '+getFontCssFromFontFamily(item.textFont)+' font-size: '+item.textSize+'px; '+_props+'}';
		cR += '#' + _id + ' .android-progress-basic-bg-bar { height: '+item.barHeight+'px; background-color: '+getColor(item.bgBarBackgroundColor)+'; }';
		if(item.isDownloading == true) {
		cR += '#' + _id + ' .android-progress-basic-percent { color: '+getColor(item.percentColor)+'; '+getFontCssFromFontFamily(item.percentFont)+' font-size: '+item.percentSize+'px; '+_percent_props+'}';
		cR += '#' + _id + ' .android-progress-basic-percent-bar { width: '+item.percentBarValue+'%; background-color: '+getColor(item.percentBarBackgroundColor)+'; }';
		}
		if(item.isDownloading == false) {
		cR += '#' + _id + ' .android-progress-basic-bg-bar-node { background-color: '+getColor(item.cubesColor)+'; width: '+item.barHeight+'px; }';
		}
		cR += '</style>';
...
prx.types.android_progressestimating = cloneobject(prx.types.android_progressbasic);
prx.types.android_progressestimating.name = 'android_progressestimating';
removeProperties(prx.types.android_progressestimating.propertyGroups, ['percent', 'percentSize', 'percentFont', 'percentColor', 'percentProperties','percentBarValue', 'percentBarBackgroundColor']);
...
prx.types.android_slider = {
	...
	,onDisplay: function(item,containerid,pageid,symbol) {
		...
		cR += '<style>'
		cR += '#'+_id+' .android-slider-bar { height: '+item.barThickness+'px; background-color: '+getColor(item.barColor)+'; margin-top: -'+(item.barThickness/2)+'px; }'
		cR += '#'+_id+' .android-slider-bar-filled { width: '+item.sliderPosition+'%; background-color: '+getColor(item.fillBarColor)+' }'
		cR += '#'+_id+' .android-slider-button { background-color: '+getColor(item.fillBarColor)+'; width: '+(item.barThickness*4)+'px; height: '+(item.barThickness*4)+'px; margin-left: -'+(item.barThickness*2)+'px; margin-right: -'+(item.barThickness*2)+'px; top: '+((_dims.height/2)-((item.barThickness*4)/2))+'px; left: '+item.sliderPosition+'%; }';
		cR += '#'+_id+' .android-slider-button-overlay {  background-color: '+getColor(item.fillBarColor)+'; }';
		cR += '</style>'
	...
	,afterDisplay: function(item,containerid,pageid,symbol) {
		...
		if(!prx.editor) {
			...
			Draggable.create('#'+_id+' .android-slider-button', {
				type: 'x',
				bounds: { left: $('#'+_id).offset().left - (item.barThickness*2), width: parseInt(_dims.width)+parseInt((item.barThickness*4)), top: 0, height: 0 },
